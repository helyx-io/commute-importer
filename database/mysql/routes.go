...
func (s MySQLRouteRepository) RemoveAllByAgencyKey(agencyKey string) (error) {
	...
	table := fmt.Sprintf("%s_routes", agencyKey)
...
func (r MySQLRouteRepository) CreateImportTask(taskName string, jobIndex int, fileName, agencyKey string, headers []string, lines []byte, workPool *workpool.WorkPool, done chan error) workpool.PoolWorker {
...
func (s MySQLRouteRepository) CreateTableByAgencyKey(agencyKey string) error {
	...
	tmpTable := fmt.Sprintf("%s_routes", agencyKey)
...
func (m MySQLRoutesImportTask) ImportModels(headers []string, as []interface{}) error {
	...
	table := fmt.Sprintf("%s_routes", m.AgencyKey)
