...
func (ac *ImportController) Import(w http.ResponseWriter, r *http.Request) {
	...
	for _, fi := range fis {
		if fi.Mode().IsRegular() {
			...
			err := gaf.ImportGTFSArchiveFileWithTableCreation(keyParam, folderFilename, gtfsModelRepository, 1024 * 1000, config.WorkPool)
			...
			if fi.Name() == "agency.txt" {
				...
				err:= gaf.ImportGTFSArchiveFileWithoutTableCreation(keyParam, folderFilename, gtfsAgencyModelRepository, 1024 * 1000, config.WorkPool)
